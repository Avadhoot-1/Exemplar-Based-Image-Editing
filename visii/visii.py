...
import sys
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'diffusers')))
from diffusers.models import AutoencoderKL, UNet2DConditionModel
# from src.diffusers.models.unets.my_unet import my_UNet2DConditionModel
# from unet_code import my_UNet2DConditionModel
...
class StableDiffusionVisii(DiffusionPipeline):
    ...
    Args:
        ...
        unet ([`my_UNet2DConditionModel`]): Conditional U-Net architecture to denoise the encoded image latents.
    ...
    def test_concatenate(
        ...
        # prompt_embeds_3 = prompt_embeds.clone()
        # torch.save(prompt_embeds_3, f'/mnt/localssd/avadhoot/ins_pt_files/{fol_name}_ct.pt')
        # bs_embed, seq_len, _ = prompt_embeds_3.shape
        # prompt_embeds_3 = prompt_embeds_3.repeat(1, num_images_per_prompt, 1)
        # prompt_embeds_3 = prompt_embeds_3.view(bs_embed * num_images_per_prompt, seq_len, -1)
        
        prompt_embeds[0, overlaping_start:(overlaping_start+eos_location_2)] = prompt_embeds_2[0, 0:eos_location_2]
        # torch.save(prompt_embeds_2, f'/mnt/localssd/avadhoot/ins_pt_files/{fol_name}_llava.pt')
        # torch.save(prompt_embeds, f'/mnt/localssd/avadhoot/ins_pt_files/{fol_name}_llava+ct.pt')
        # exit(0)
        bs_embed, seq_len, _ = prompt_embeds.shape
        prompt_embeds = prompt_embeds.repeat(1, num_images_per_prompt, 1)
        prompt_embeds = prompt_embeds.view(bs_embed * num_images_per_prompt, seq_len, -1)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # predict the noise residual
                # print(self.unet)
                # print(self.unet(scaled_latent_model_input, t, encoder_hidden_states=prompt_embeds_2))
                # exit(0)
                noise_pred = self.unet(scaled_latent_model_input, t, encoder_hidden_states=prompt_embeds).sample
                # Hack:
                # For karras style schedulers the model does classifer free guidance using the
